...
module Outgoing_socket = struct
  type t =
    ...
    ; xmit_queue: (int * (Lwt_bytes.t -> int)) Lwt_queue.t
    ; switch_off_promise : [`Closed] Lwt.t
    ; switch: Lwt_switch.t }
  ...
  let send t msg =
    if Lwt_switch.is_on t.switch then (
      ...
      try
        ...
        Lwt_queue.add t.xmit_queue (size, cont) ;
        Ok ()
  ...
  let send_thread t =
    ...
    in
    let rec loop () =
      let msg_p = Lwt_queue.take t.xmit_queue >>>= fun v -> Lwt.return_ok v in
      let timeout_p = t.switch_off_promise >|= fun v -> 
        Log.warn (fun m -> m "Switch closed exiting send loop");
        Error v 
      in
      Lwt.choose [msg_p; timeout_p]
      >>>= fun (size, cont) ->
  ...
  let create ?switch fd =
    ...
    Lwt_switch.add_hook (Some switch) (fun () ->
        Lwt.wakeup off_f `Closed;
        Lwt.return_unit) ;
    let xmit_queue = Lwt_queue.create () in
    let t = {fd; xmit_queue; switch_off_promise=off_p; switch} in
    Lwt.async (fun () ->
        send_thread t
        >>= function
        | Ok () ->
